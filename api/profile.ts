...
import { sendJson, methodNotAllowed, readBody, validateName, JLPT_LEVELS, sendApiError } from './_lib/http.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      const user = result as unknown as import('./_lib/user.js').UserDoc | null
      if (!user) return sendJson(res, 404, { error: 'User not found' })
      return sendJson(res, 200, { user: toPublicUser(user) })
    }
    ...
    return sendApiError(res, 'profile', err)
  }
}
