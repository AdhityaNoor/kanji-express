...
export default function Write() {
  ...
  // Built-in fallback (works offline / if the API is unreachable).
  ...
  // Load the full JLPT kanji set for the chosen level.
  useEffect(() => {
    let active = true
    setListLoading(true)
  ...
  // Load readings / meanings / example words for the selected kanji.
  useEffect(() => {
    let active = true
  ...
  return (
    <div className="animate-fade-up space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-fg-strong sm:text-3xl">Handwriting practice</h1>
          ...
          <CardBody className="p-5 sm:p-6">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="grid h-14 w-14 place-items-center rounded-xl bg-accent/10 font-jp text-3xl font-bold text-accent-fg">
                  {selected}
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <SpeakButton text={selected} icon />
                  </div>
                  ...
                  <ul className="space-y-1.5">
                    {words.map((w, i) => (
                      <li key={i} className="flex items-center gap-2 rounded-lg bg-bg-soft px-3 py-1.5">
                        ...
                        <SpeakButton text={w.written} icon />
                      </li>
                    ))}
                  </ul>
                ) : (
              ...
              <div className="grid max-h-[460px] grid-cols-6 gap-1.5 overflow-y-auto sm:grid-cols-7 lg:grid-cols-6">
                {chars.map((c) => (
                  <button
                    ...
                    className={cn(
                      'grid aspect-square place-items-center rounded-lg font-jp text-lg font-bold transition-colors',
